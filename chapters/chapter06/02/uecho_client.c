...
#define BUF_SIZE (1024)
    ...
    // ready
    my_socket(local_sock, PF_INET, SOCK_DGRAM, 0);
    ...
    while (true)
    {
        ...
        my_sendto(local_sock, msg_buf, strlen(msg_buf), 0, (struct sockaddr *)&serv_addr, sizeof(serv_addr));
        ssize_t msg_len;
        static struct sockaddr_in from_addr;
        socklen_t from_addr_size = sizeof(from_addr);
        my_recvfrom(msg_len, local_sock, msg_buf, sizeof(msg_buf), 0, (struct sockaddr *)&from_addr, &from_addr_size);
        msg_buf[msg_len] = '\0';
